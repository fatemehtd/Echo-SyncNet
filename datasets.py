...
# from train.dataset_splits import DATASETS
from preprocessors import preprocess_sequence

DATASETS = {
    'AP4':{'train': 4352, 'val': 1377}
}


NUM_PARALLEL_CALLS = 8 #Number of parallel calls while preprocessing data on CPU
...
def decode(serialized_example):
  ...
  if CONFIG.OPTICALFLOW:
      seq_features['rgb_video'] =  tf.io.FixedLenSequenceFeature([], dtype=tf.string)

  if CONFIG.DATA.FRAME_LABELS:
    if CONFIG.EVAL.LINEAR_MODEL_TYPE == 'svr':
  ...
  if CONFIG.OPTICALFLOW:
    rgb_video =  sequence_data.get('rgb_video', [])
  else:
      rgb_video = []
  ...
  if CONFIG.OPTICALFLOW:
    return video, rgb_video, frame_labels, seq_label, seq_len, name
  else:
...
def sample_and_preprocess_optical(video,
                          rgb_video,
                          labels,
                          seq_label,
                          seq_len,
  ...
  rgb_video = tf.gather(rgb_video, steps)
  ...
  rgb_video = preprocess_input(rgb_video, augment)

  if add_shape:
    video.set_shape([shape_all_steps, CONFIG.IMAGE_SIZE, CONFIG.IMAGE_SIZE, 3])
    rgb_video.set_shape([shape_all_steps, CONFIG.IMAGE_SIZE, CONFIG.IMAGE_SIZE, 3])
...
def get_tfrecords(dataset, split, path, per_class=False):
  ...
  if per_class:
    ...
    path_to_tfrecords = os.path.join(path % dataset if '%s' in path else path,
                                     '%s_%s*' % (dataset, split))
...
def create_dataset(split, mode, batch_size=None, return_iterator=True):
  ...
  if mode == 'train':
    ...
    if CONFIG.OPTICALFLOW:
        preprocess_fn = (
            lambda video, rgb_video, labels, seq_label, seq_len, name: sample_and_preprocess_optical(
                video,
                rgb_video,
                labels,
                seq_label,
  ...
  with tf.device('/cpu:0'):
    for dataset_name in CONFIG.DATASETS:
      ...
      datasets_size.append(len(tfrecord_files))
    ...
    # Prefetch batches
    dataset = dataset.prefetch(CONFIG.DATA.NUM_PREFETCH_BATCHES)
    
    # since we return size of the dataset, only ONE dataset is supported
    assert len(CONFIG.DATASETS) == 1, "multiple datasets is not supported"

    if return_iterator:
      return iter(dataset), math.floor(datasets_size[0]/batch_size)
    else:
      return dataset, math.floor(datasets_size[0]/batch_size)

@tf.autograph.experimental.do_not_convert
def create_one_epoch_dataset(dataset, split, mode, path_to_tfrecords=None):
  """Creates a dataset iterator that gives one epoch of dataset."""
  batch_size = 1
  sample_all_stride = CONFIG.DATA.SAMPLE_ALL_STRIDE
  tfrecord_files = get_tfrecords(dataset, split, CONFIG.PATH_TO_TFRECORDS \
    if path_to_tfrecords is None else path_to_tfrecords)
  ...
  return iter(dataset), math.floor(len(tfrecord_files) / batch_size)
